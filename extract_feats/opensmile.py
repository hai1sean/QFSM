...
def get_feature_opensmile(config, filepath: str) -> list:
    ...
    # Opensmile 命令
    cmd = 'cd ' + config.opensmile_path + ' && '+ config.opensmile_path + '/SMILExtract -C ' + opensmile_config_path + ' -I ' + filepath + ' -O ' + single_feat_path + ' -appendarff 0'
...
def load_feature(config, train: bool) -> Union[Tuple[np.ndarray], np.ndarray]:
    ...
    # split_X = np.split(X, 3600, axis=0)
    # con_X = []
    # for i in range(0, len(split_X), 3):
    #     X_2 = np.concatenate((split_X[i], split_X[i+1], split_X[i+2]), axis=0)
    #     con_X.append(X_2)
    # con_X_arrary = np.stack(con_X, axis=0)
    ...
    if train == True:
        ...
        scaler_2 = MinMaxScaler().fit(X)
        X = scaler_2.transform(X)
        pca = PCA(n_components=64)
        X_pca = pca.fit_transform(X)
        X_pca_reshaped = X_pca.reshape(-1, 5, 64)



        # 划分训练集和测试集
        x_train, x_test, y_train, y_test = train_test_split(X_pca_reshaped, split_Y, test_size = 0.2, random_state = 42)
...
def get_data(config, data_path: str, train: bool) -> Union[Tuple[np.ndarray], np.ndarray]:
    ...
    writer.writerow(first_row)   # 写第一行，对应特征维度
