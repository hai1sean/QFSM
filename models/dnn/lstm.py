...
class LSTM(DNN):
    ...
    def make(
        ...
        model.add(KERAS_LSTM(rnn_size, input_shape=(5, 64), kernel_regularizer=regularizers.l1(0.01)))  # (time_steps = 1, n_feats)
    ...
    def reshape_input(self, data: np.ndarray) -> np.ndarray:
        ...
        data = np.reshape(data, (data.shape[0], 5, data.shape[2]))
        return data
